...
async function fetchExchangeRates(
  ...
  for (const source of API_SOURCES) {
    try {
      ...
      if (!response.ok)
        throw new Error(`API error ${source.name}: ${response.status}`);
...
async function recordFailure(
  baseCurrency: string,
  existing?: FailureRecord | null,
): Promise<void> {
  const stored = await browser.storage.local.get(FAILURE_RECORD_KEY);
  const records =
    (stored[FAILURE_RECORD_KEY] as Record<string, FailureRecord> | undefined) ??
    {};
